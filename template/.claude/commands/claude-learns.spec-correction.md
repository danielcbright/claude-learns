# /claude-learns.spec-correction - Capture Corrections When Claims Were Wrong
...
2. Run `/claude-learns.spec-verify` with full evidence
...
When `/claude-learns.spec-verify` runs:
...
Then run `/claude-learns.spec-correction` to persist it.
...
Persistent record of premature completion claims. Read by /claude-learns.spec-verify to
