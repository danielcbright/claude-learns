...
These are checked during /spec-verify and debugging sessions.
...
- [target]/.claude/update-registry.yaml (for /update command)
...
- /update --list      → Should show registered tools
- /learn              → Should show "No learnings to capture" or routing options
- /eliminate-status   → Should show "No active investigation"
- /spec-list          → Should show specs directory (may be empty)
```
...
4. **Suggest /audit later** - to re-check after fixes
