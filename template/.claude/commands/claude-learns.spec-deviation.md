# /claude-learns.spec-deviation - Log Intentional Spec Deviation

**Syntax:** `/claude-learns.spec-deviation [feature_name] [--reason=<category>]`
...
1. `/claude-learns.spec-validate` doesn't flag them as bugs
...
2. **Notify validation**
   - Deviation file will be read by `/claude-learns.spec-validate`
...
```
User: /claude-learns.spec-deviation user-authentication --reason=performance_optimization
...
The deviation will now be recognized by /claude-learns.spec-validate and won't be flagged as a bug.
...
- **Read by `/claude-learns.spec-validate`**: Prevents false positive bug detection
- **Read by `/claude-learns.spec-debug`**: Distinguishes bugs from intentional changes
