...
**Syntax:** `/claude-learns.evidence [hypothesis_id] [observation] [--supports|--contradicts] [--weight 0.X]`
...
/claude-learns.evidence H3 "Lock contention detected in handler logs" --supports --weight 0.8

# Evidence contradicting a hypothesis  
/claude-learns.evidence H1 "Connection pool at 30% capacity during errors" --contradicts --weight 0.9

# Evidence affecting multiple hypotheses
/claude-learns.evidence H2,H5 "Memory stable at 2GB for 24 hours" --contradicts

# With source reference
/claude-learns.evidence H3 "Race condition in SharedCache.get()" --supports --source "src/cache.ts:142"
```
...
```
User: /claude-learns.evidence H1 "Database connections at 45/100 during error spike" --contradicts --weight 0.85 --source "grafana:db-pool-dashboard"
