# /update - Update Tools and Template
...
```
/update                     Check all tools for updates (dry run)
/update --list              List registered tools and their status
/update [tool-name]         Update specific tool (e.g., serena, context7, claude-learns)
/update --all               Update all registered tools
/update --rollback          Rollback to last checkpoint
```
...
1. Run /audit to verify documentation consistency
...
# Add new tools here to include them in /update checks.
...
2. Add entry under `tools:`:
   ...
   new-tool:
     ...
     installed_version: null
   ```
3. Run `/update new-tool` to fetch and install
...
Run `/update` first to create a checkpoint before trying to rollback.
...
Add the tool to `.claude/update-registry.yaml` manually, then run `/update [tool-name]`.
...
Rollback available: /update --rollback
...
Changes:
- New command: /foo
- Updated: /learn (improved routing)
