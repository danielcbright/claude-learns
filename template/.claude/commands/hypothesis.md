...
**Syntax:** `/hypothesis [description] [--confidence 0.X] [--domain category]`
...
1. **Check for active session**
   - If no session exists, prompt to start `/eliminate` first
...
Run `/evidence` after testing to record results.
```

## Integration

- Works with active `/eliminate` session
- Hypothesis appears in `/eliminate-status` output
- Evidence recorded via `/evidence` updates this hypothesis
