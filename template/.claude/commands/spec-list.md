...
**Syntax:** `/spec-list [--status=<status>] [--type=<type>] [--verbose]`
...
Use `/spec-validate [name]` to check implementation against spec.
Use `/spec-create [name]` to create a new specification.
```

```
User: /spec-list --status=draft --verbose
...
- **Links to `/spec-validate`**: Suggests validation for implemented specs
- **Links to `/spec-create`**: Offers to create new specs
