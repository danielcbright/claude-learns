...
**Syntax:** `/spec-create [feature_name] [--type=feature|bug-fix|refactor]`
...
3. **Generate summary**
   ...
   **Status**: draft
   ...
   3. Validate with `/spec-validate {feature_name}`
   ```

## Example

```
User: /spec-create user-authentication --type=feature
...
3. Validate with `/spec-validate user-authentication`
```

## Integration Points

- **With `/eliminate`**: Specs provide expected behavior for hypothesis generation
- **With `/spec-debug`**: Debugging uses spec as source of truth
