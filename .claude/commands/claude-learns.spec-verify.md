# /claude-learns.spec-verify - Verification Gate Before Claiming Completion
...
- Fix each issue
- Re-run `/claude-learns.spec-verify` until all pass
...
1. **Immediately run `/claude-learns.spec-correction`** to capture what went wrong
2. The correction gets added to `.specify/memory/corrections.md`
3. Future `/claude-learns.spec-verify` runs will check for this pattern
...
/claude-learns.spec-verify login-feature
