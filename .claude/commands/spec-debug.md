# /spec-debug - Debug Using Specification as Source of Truth

**Syntax:** `/spec-debug [feature_name] [symptom_description]`
...
1. **If spec deviation was the cause**
   ...
   - Update spec if implementation is correct (with `/spec-deviation`)
...
```
User: /spec-debug user-authentication "login fails with correct credentials"
...
| Scenario | Command |
|----------|---------|
| Bug in specced feature | `/spec-debug` |
| Bug without spec | `/debug` or `/eliminate` |
| Complex multi-cause issue | `/eliminate` (may call `/spec-debug` internally) |
| Validating implementation | `/spec-validate` |

## Integration Points

- **Calls `/eliminate`**: For the evidence gathering and elimination phases
- **Uses specs**: Reads from `.specify/specs/`
- **Updates memories**: Writes to `debugging-lessons`, `common-bugs`
- **May trigger `/spec-deviation`**: If intentional deviation found
