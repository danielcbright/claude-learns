# /claude-learns.spec-validate - Validate Implementation Against Spec

**Syntax:** `/claude-learns.spec-validate [feature_name] [--strict] [--generate-report]`
...
1. **Read spec file**
   ...
   - If not found, suggest creating with `/claude-learns.spec-create`
...
1. **Classify deviations**
   ...
   | Undocumented | Intentional but not logged | Log with `/claude-learns.spec-deviation` |
...
```
User: /claude-learns.spec-validate user-authentication
...
- **Feeds into `/claude-learns.spec-debug`**: Validation failures become debugging hypotheses
- **Triggers `/claude-learns.spec-deviation`**: Undocumented changes need deviation logs
